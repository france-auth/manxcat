...
type DropdownProps = {
  onLanguageChange: (language: string) => void; // Prop for handling language change
};
...
const Dropdown: React.FC<DropdownProps> = ({ onLanguageChange }) => {
  ...
  const handleSelect = (language: string) => {
    setSelectedLanguage(language);
    setIsOpen(false); // Close the dropdown after selection
    onLanguageChange(language); // Invoke callback to update language
  };

  // Filter out the selected language from the dropdown list
  const filteredLanguages = languages.filter((language) => language !== selectedLanguage);

  return (
    <div className="flex items-center w-full rounded-xl relative">
      <div
        className="w-full flex flex-row-reverse gap-3 justify-between items-center px-4 py-2 border border-[#000807] bg-[#EB8A90] rounded-xl"
        onClick={toggleDropdown}
      >
        <div className="flex items-center cursor-pointer space-x-3">
          <svg
      ...
      >
        {filteredLanguages.map((language, id) => (
          <div
            key={id}
            className="flex justify-start py-2 px-4 mt-1 cursor-pointer"
            onClick={() => handleSelect(language)}
